...
class imdb:
    ...
    def parse_data(self):
        ...
        try:
            ...
            test = filter(lambda movie: movie['Source'] == 'Rotten Tomatoes', list_of_movies)
            if test:
                print "Rotten Tomatoes Ratings -->" + \
                    map(str, [dic['Value'] for dic in test])[0]
            else:
                print "Rotten Tomatoes Ratings not present, Displaying others rating \n"
                print map(str, [dic['Source'] for dic in list_of_movies])[0], map(str, [dic['Value'] for dic in list_of_movies])[0]
        except KeyError:
            pass
